 ...
 * {@link Inventory} {@link IMenu}s. Menu inventory listening is delegated and centralized in one {@link MenuManager}
...
public class MenuManager implements Listener {
    ...
    /**
     * The {@link Plugin} that registers this {@link Listener}'s event handlers.
     */
    @Getter
     ...
     *
     * @param plugin  Not null enabled {@link Plugin} for registering this {@link Listener} instance's event handlers
     ...
     */
    protected MenuManager(Plugin plugin, Map<Inventory, IMenu> mapImpl) {
        Preconditions.checkState(plugin.isEnabled(), "Plugin provider argument can't be disabled");
     ...
     */
    public Optional<IMenu> unregisterMenu(Inventory inventory) {
     ...
     */
    public Optional<IMenu> unregisterMenu(IMenu menu) {
     ...
     * @throws IllegalArgumentException If the {@link IMenu} argument is null
     */
    public Optional<IMenu> registerMenu(IMenu menu) {
    ...
    public MenuManager closeMenus() {
        List.copyOf(this.menus.values()).forEach(IMenu::close);
        return this;
    }
     ...
     * @throws IllegalArgumentException If the {@link Inventory} argument is null
     */
    public Optional<IMenu> getMenu(Inventory inventory) {
     ...
     */
    public Optional<IMenu> getMenu(HumanEntity viewer) {
    ...
    /**
     * Passes a {@link PluginDisableEvent} of this {@link MenuManager}'s provider ({@link #plugin()}) to all inventory
     * menus.
     *
            ...
            for (final var iterator = this.menus.entrySet().iterator(); iterator.hasNext(); ) {
                try {
                    ...
                    this.plugin().getSLF4JLogger()
                        .warn("An error occurred while handling a menu plugin disable event", exception);
                }
            }
